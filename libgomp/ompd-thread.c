...
ompd_get_thread_in_parallel (ompd_parallel_handle_t *parallel_handle,
			     int thread_num, ompd_thread_handle_t
			     **thread_handle)
{
  ...
  taddr.address += field_offset;

  ret = callbacks->read_memory (context, NULL, &taddr,
  				target_sizes.sizeof_pointer,
				&(temp_addr.address));
  ret = callbacks->device_to_host (context, &(temp_addr.address),
  				   target_sizes.sizeof_pointer, 1,
				   &(taddr.address));
...
ompd_get_thread_handle (ompd_address_space_handle_t *handle,
		        ...
		        const void *thread_id, ompd_thread_handle_t
		        **thread_handle)
{
  if (handle == NULL)
    return ompd_rc_stale_handle;
  ...
  ompd_address_t temp_addr, symbol_addr = {OMPD_SEGMENT_UNSPECIFIED, 0};
